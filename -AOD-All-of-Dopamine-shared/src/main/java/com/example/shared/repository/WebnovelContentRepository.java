...
public interface WebnovelContentRepository extends JpaRepository<WebnovelContent, Long> {
     ...
     */
    @Query(value = "SELECT g AS genre, COUNT(*) AS cnt FROM webnovel_contents, UNNEST(genres) AS g " +
     ...
     * 사용 중인 장르 목록 (중복 제거)
     */
    @Query(value = "SELECT DISTINCT g FROM webnovel_contents, UNNEST(genres) AS g " +
