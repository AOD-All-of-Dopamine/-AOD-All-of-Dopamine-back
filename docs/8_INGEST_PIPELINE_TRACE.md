...
genres      → domain.genres               WebnovelContent.genres = ["판타지"]
...
> ⚠️ **알려진 이슈**: 도메인 덮어쓰기가 `platforms` 배열도 교체한다 → 크로스플랫폼 병합 시 기존 플랫폼 유실 가능. 구 시스템과 동일한 동작이라 보존 원칙에 따라 그대로 두고 별도 이슈로 다룬다 (`IngestPipeline.mergeInto` 주석 참고).
