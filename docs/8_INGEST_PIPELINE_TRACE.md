...
4. `master.platforms` 주입: `[자기 플랫폼명]` + `platformsFrom` 선언 키의 attributes 값 병합 — yml 매핑과 무관하게 **항상 채움** (RF-4, 2026-07 마스터로 승격). platforms가 mappings에 없는 이유가 이것 (원본에서 안 오고 엔진이 만듦).
...
- **Content**: null인 필드만 채움 (기존 값 우선) — 예외 둘: genres는 값이 오면 덮어쓰기(재수집 갱신), platforms는 기존∪신규 합집합(크로스플랫폼 유실 방지)
...
- **도메인 필드**: `boundDomainProps`에 든 프로퍼티만 **덮어쓰기**
