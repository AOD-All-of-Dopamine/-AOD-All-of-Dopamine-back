...
- [ ] **Step 0.2: 테스트 실행으로 contextLoads 복구 확인**
...
- [ ] **Step 1.1: TmdbService의 큐용 메서드를 Executor로 이동**
...
- [ ] **Step 1.2: 죽은 컨트롤러 + 레거시 Service 삭제**
...
- [ ] **Step 2.1: SteamCrawlService.collectGameByAppId → SteamGameExecutor로 이동**

Phase 1의 Step 1.1과 동일 패턴: 본문 무변경 이동, 필드 추가, `execute()`에서 호출.

- [ ] **Step 2.2: admin.html이 쓰는 두 경로를 큐 방식으로 대체**
...
- [ ] **Step 3.1: HtmlParseUtils 신설 (이동, 재작성 아님)**
...
- [ ] **Step 3.2: NaverSeriesSelectors 신설 + 인라인 셀렉터 전량 교체**
...
- [ ] **Step 4.1: 인라인 셀렉터를 NaverWebtoonSelectors로 편입**
...
- [ ] **Step 4.2: 발견 로직을 Fetcher로, 메트릭을 Executor로**
...
- [ ] **Step 5.2: 최종 성공 기준 grep 게이트 (스펙 §6)**
