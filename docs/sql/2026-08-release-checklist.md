...
## 2. back 배포 (스키마·백필은 Flyway가 자동 수행 - 2026-08-17 배선)

api 기동 시 Flyway가 실행된다 (`FlywayConfig`: baseline-on-migrate, baseline-version=3):
- **V4**: collections / collection_items / collection_likes 생성 (prod는 ddl-auto가 없으므로 이게 유일한 스키마 경로)
- **V5**: genres/platforms 승격 백필 (구 수동 SQL §1~3의 Flyway 이관 - 멱등)

확인:
```
기동 로그: "Successfully applied N migrations" 또는 "Schema ... is up to date"
GET /api/collections?size=1        -> 200
...
(구 수동 절차였던 `docs/sql/2026-07-promote-*.sql` §1~3 직접 실행은 더 이상 불필요.
 §4 DROP은 여전히 검증 후 별도 마이그레이션으로.)
